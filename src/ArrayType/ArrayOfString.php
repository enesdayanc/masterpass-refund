...
class ArrayOfString extends AbstractStructArrayBase
{
     ...
     * @uses ArrayOfString::setAccount_name()
     ...
     * @param string[] $account_name
     * @return \Enesdayanc\MasterpassRefund\ArrayType\ArrayOfString
     */
    public function setAccount_name(array $account_name = array())
    {
        foreach ($account_name as $arrayOfStringAccount_nameItem) {
            // validation for constraint: itemType
            if (!is_string($arrayOfStringAccount_nameItem)) {
                throw new \InvalidArgumentException(sprintf('The account_name property can only contain items of string, "%s" given', is_object($arrayOfStringAccount_nameItem) ? get_class($arrayOfStringAccount_nameItem) : gettype($arrayOfStringAccount_nameItem)), __LINE__);
            }
        }
