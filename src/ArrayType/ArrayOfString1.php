...
class ArrayOfString1 extends AbstractStructArrayBase
{
     ...
     * @uses ArrayOfString1::setAddress_name()
     ...
     * @param string[] $address_name
     * @return \Enesdayanc\MasterpassRefund\ArrayType\ArrayOfString1
     */
    public function setAddress_name(array $address_name = array())
    {
        foreach ($address_name as $arrayOfString1Address_nameItem) {
            // validation for constraint: itemType
            if (!is_string($arrayOfString1Address_nameItem)) {
                throw new \InvalidArgumentException(sprintf('The address_name property can only contain items of string, "%s" given', is_object($arrayOfString1Address_nameItem) ? get_class($arrayOfString1Address_nameItem) : gettype($arrayOfString1Address_nameItem)), __LINE__);
            }
        }
