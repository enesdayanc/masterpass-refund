...
class ResponseBody extends AbstractStructBase
{
     ...
     * - maxOccurs: unbounded
     * - minOccurs: 0
     * @var \Enesdayanc\MasterpassRefund\StructType\SmsNotification[]
     */
    public $sms_noti;
    /**
     ...
     * @param \Enesdayanc\MasterpassRefund\StructType\SmsNotification[] $sms_noti
     */
     ...
     * @param string $retrieval_reference_no
     * @return \Enesdayanc\MasterpassRefund\StructType\ResponseBody
     */
    ...
    /**
     * Get sms_noti value
     * @return \Enesdayanc\MasterpassRefund\StructType\SmsNotification[]|null
     */
    ...
    public function setSms_noti(array $sms_noti = array())
    {
        foreach ($sms_noti as $responseBodySms_notiItem) {
            ...
            if (!$responseBodySms_notiItem instanceof \Enesdayanc\MasterpassRefund\StructType\SmsNotification) {
                throw new \InvalidArgumentException(sprintf('The sms_noti property can only contain items of \Enesdayanc\MasterpassRefund\StructType\SmsNotification, "%s" given', is_object($responseBodySms_notiItem) ? get_class($responseBodySms_notiItem) : gettype($responseBodySms_notiItem)), __LINE__);
            }
        }
     ...
     * @throws \InvalidArgumentException
     * @param \Enesdayanc\MasterpassRefund\StructType\SmsNotification $item
     * @return \Enesdayanc\MasterpassRefund\StructType\ResponseBody
     */
    public function addToSms_noti(\Enesdayanc\MasterpassRefund\StructType\SmsNotification $item)
    {
        // validation for constraint: itemType
        if (!$item instanceof \Enesdayanc\MasterpassRefund\StructType\SmsNotification) {
            throw new \InvalidArgumentException(sprintf('The sms_noti property can only contain items of \Enesdayanc\MasterpassRefund\StructType\SmsNotification, "%s" given', is_object($item) ? get_class($item) : gettype($item)), __LINE__);
        }
        $this->sms_noti[] = $item;
        return $this;
    }
